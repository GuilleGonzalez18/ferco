/**
 * Bootstrap script: creates src/features/configuracion/ with its initial files
 * if they do not already exist. Runs automatically via prebuild / predev hooks.
 */
...
const dir = join(__dirname, 'src', 'features', 'configuracion');

if (!existsSync(dir)) {
  mkdirSync(dir, { recursive: true });
  console.log('[setup] Created src/features/configuracion/');
}

const jsxPath = join(dir, 'Configuracion.jsx');
if (!existsSync(jsxPath)) {
  writeFileSync(jsxPath, getConfiguracionJsx());
  console.log('[setup] Created Configuracion.jsx');
}

const cssPath = join(dir, 'Configuracion.css');
if (!existsSync(cssPath)) {
  writeFileSync(cssPath, getConfiguracionCss());
  console.log('[setup] Created Configuracion.css');
}

// ─────────────────────────────────────────────────────────────────────────────
function getConfiguracionJsx() {
