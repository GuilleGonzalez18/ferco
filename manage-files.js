...
const TARGET_DIR = 'D:/repos/ferco-posta/frontend/src/features/configuracion';
...
// ── Step 2: Write Configuracion.jsx ──────────────────────────────────────────
const jsxContent = `import { useCallback, useEffect, useRef, useState } from 'react';
...
try {
  fs.writeFileSync(TARGET_DIR + '/Configuracion.jsx', jsxContent, 'utf8');
  console.log('✓ Created: Configuracion.jsx');
} catch (err) {
  console.log('✗ Failed to create Configuracion.jsx:', err.message);
}

// ── Step 3: Write Configuracion.css ──────────────────────────────────────────
const cssContent = `.config-root {
  display: flex;
...
.config-tab-btn {
  ...
  font: var(--weight-medium) var(--font-md) var(--font-sans);
...
.config-section-title {
  font: var(--weight-semibold) var(--font-lg) var(--font-heading);
...
.config-field-label {
  font: var(--weight-medium) var(--font-sm) var(--font-sans);
  color: var(--color-text-muted);
  padding-top: 10px;
}
...
.config-toggle-input:disabled ~ .config-toggle-track {
  opacity: 0.5;
  cursor: not-allowed;
}
...
try {
  fs.writeFileSync(TARGET_DIR + '/Configuracion.css', cssContent, 'utf8');
  console.log('✓ Created: Configuracion.css');
} catch (err) {
  console.log('✗ Failed to create Configuracion.css:', err.message);
}

// ── Step 4: Delete misplaced files ────────────────────────────────────────────
const filesToDelete = [
  'D:/repos/ferco-posta/frontend/src/features/dashboard/Configuracion.jsx',
  'D:/repos/ferco-posta/frontend/src/features/dashboard/Configuracion.css',
  'D:/repos/ferco-posta/frontend/setup-configuracion.js',
  'D:/repos/ferco-posta/frontend/src/features/setup-config.js',
  'D:/repos/ferco-posta/manage-files.js',
];
...
console.log('\nDone. Verify:');
console.log(' ', TARGET_DIR + '/Configuracion.jsx');
console.log(' ', TARGET_DIR + '/Configuracion.css');
